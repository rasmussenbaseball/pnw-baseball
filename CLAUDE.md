...
│   │   ├── pnw_grid.json         # Gitignored, daily grid puzzle regenerated by the API
│   │   └── park_factors.json
│   └── requirements.txt
├── frontend/
│   ├── .env                      # VITE_SUPABASE_URL, VITE_SUPABASE_ANON_KEY
│   ├── src/
│   │   ├── App.jsx               # Router with 100+ routes
│   │   ├── pages/                # ~100 page components (+ gm/, portal/, summer/ subdirs)
...
**CORS:** narrowed June 2026 — explicit method + header lists in main.py (the one cross-origin need is image uploads to api.nwbaseballstats.com). If a new cross-origin call fails preflight, add its method/header there.
...
The current season has ONE source of truth per layer (consolidated June 2026):
- **Frontend:** `frontend/src/lib/seasons.js` exports `CURRENT_SEASON`, `SEASONS`, `clampSeason`. (`utils/constants.js` re-exports `CURRENT_SEASON` for older imports.)
- **Backend:** `backend/app/config.py` exports `CURRENT_SEASON`, used as the `season` default in API endpoints.
...
The player profile page (`frontend/src/pages/PlayerDetail.jsx`) is the most heavily designed surface on the site.

**Redesigned template (May 2026):** `PlayerProfileHitter.jsx` / `PlayerProfilePitcher.jsx` + `components/playerProfile/shared.jsx` are the newer savant-style profile (cream/navy/maroon palette, JS-object theming). They're rendered through `PlayerDetail.jsx` for gated players, with a plan to roll out site-wide. NOTE: their palette is intentionally separate from the nw-teal site tokens — decide whether to promote it into tailwind.config.js before copying it to more pages (SummerPlayerProfile already inherited it).

Key elements of the classic page:
