...
@router.get("/pnw-draft")
@cached_endpoint(ttl_seconds=3600)
def pnw_draft():
    """Data feed for the PNW Draft game (82-0 / 162-0 style roster builder).

    Reads the curated backend/data/pnw_draft.json (generated by
    scripts/build_pnw_draft_json.py from the live DB) and enriches it with a
    short_name -> logo_url map so the frontend can show team logos. The JSON
    already carries level-adjusted talent scores + win-mapping constants, so
    the frontend just runs the lightweight sim against them."""
    import json as _json
    from pathlib import Path as _Path

    path = _Path(__file__).parent.parent.parent / "data" / "pnw_draft.json"
    if not path.exists():
        raise HTTPException(status_code=404, detail="PNW Draft data not available")
    with open(path) as f:
        payload = _json.load(f)

    # Logo lookup for every team referenced in the pool.
    shorts = sorted({s for lst in payload.get("teams", {}).values() for s in lst})
    logos = {}
    if shorts:
        with get_connection() as conn:
            cur = conn.cursor()
            cur.execute(
                "SELECT short_name, logo_url FROM teams WHERE short_name = ANY(%s)",
                (shorts,),
            )
            for r in cur.fetchall():
                if r["logo_url"]:
                    logos[r["short_name"]] = r["logo_url"]
    payload["logos"] = logos
    return payload
