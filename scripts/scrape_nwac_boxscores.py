    ...
    with get_connection() as conn:
        ...
        for side, team_id, pitching in [
            ...
            for p in pitching:
                ...
                # SAVEPOINT so a UniqueViolation on the partial
                # uniq_game_pitching_game_player index (game_id, player_id)
                # doesn't abort the whole game transaction. See
                # scrape_boxscores.py for the full rationale.
                ...
                try:
                    ...
                    logger.warning(
                        "  Skipping duplicate game_pitching row: game_id=%s "
                        "player_id=%s team_id=%s player_name=%s constraint=%s",
                        game_id, player_id, team_id, p.get("player_name"),
                        e.diag.constraint_name,
                    )
                else:
