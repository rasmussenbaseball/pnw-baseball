...
export const STAT_DEFS = {
  ...
  pa: ['Plate appearances, from the scorer fields in the TrackMan file.', 'Every PA that ended: K, BB, HBP, sacrifice or ball in play.', 0],
  ab: ['At-bats.', 'PA minus walks, hit-by-pitches and sacrifices (TrackMan cannot split sac flies from bunts, so every sacrifice is treated like a sac fly).', 0],
  h: ['Hits.', 'Singles + doubles + triples + home runs from the PlayResult field.', 0],
  d2: ['Doubles.', 'PlayResult = Double.', 0],
  d3: ['Triples.', 'PlayResult = Triple.', 0],
  hr: ['Home runs.', 'PlayResult = HomeRun.', 0],
  bb: ['Walks.', 'KorBB = Walk.', 0],
  k: ['Strikeouts.', 'KorBB = Strikeout.', 0],
  hbp: ['Hit by pitch.', 'PitchCall = HitByPitch.', 0],
  avg: ['Batting average.', 'Hits / at-bats.', 3],
  obp: ['On-base percentage.', '(H + BB + HBP) / (AB + BB + HBP + sacrifices).', 3],
  slg: ['Slugging.', 'Total bases / at-bats.', 3],
  ops: ['OPS.', 'OBP + SLG.', 3],
  iso: ['Isolated power: extra bases per at-bat.', 'SLG minus AVG.', 3],
  ...
  hr_allowed: ['Home runs allowed.', 'PlayResult = HomeRun against him.', 0],
  bb_allowed: ['Walks allowed.', 'KorBB = Walk against him.', 0],
  k_pitched: ['Strikeouts.', 'KorBB = Strikeout.', 0],
  hbp_allowed: ['Hit batters.', 'PitchCall = HitByPitch.', 0],
  whip: ['Walks plus hits per inning.', '(BB + H) / IP.', 2],
  baa: ['Batting average against.', 'Hits allowed / at-bats against.', 3],
  fip: ['Fielding-independent pitching: what his walks, strikeouts, hit batters and homers say his runs allowed should be, on the RA/9 scale of this corpus (earned runs are unknown).', '(13 HR + 3 (BB + HBP) minus 2 K) / IP + corpus constant.', 2],
  k9: ['Strikeouts per nine innings.', '9 x K / IP.', 1],
  bb9: ['Walks per nine innings.', '9 x BB / IP.', 1],
  ra9: ['Runs allowed per nine innings (R, not ER).', '9 x R / IP.', 2],
  ...
  // ── pitching ──
  stuff: ['Stuff+: physical pitch quality only (velocity, movement, spin, release, and how the pitch separates from the fastball). Says nothing about command.', 'The site-wide NWBB Stuff+ model, trained on college TrackMan pitches to predict expected run value: whiffs, called strikes, balls, and what each ball in play was worth by exit velocity and launch angle. Sliders are not graded on sweep; a gyro slider and a sweeper can both grade well. 100 = an average pitch of that type in college, 15 points per standard deviation. A pitch type with a single tracked pitch gets no grade; weigh the rest by the N column.', 0],
