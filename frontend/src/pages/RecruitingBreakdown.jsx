...
const COLUMNS = [
  ...
  { key: 'ranking', label: 'Rank', sortable: true, tooltip: 'National composite rank (D1-NAIA) or CPI rank (NWAC)' },
...
export default function RecruitingBreakdown() {
  ...
  const filtered = useMemo(() => {
    ...
    rows.sort((a, b) => {
      let aVal, bVal
      if (sortKey === 'ranking') {
        // Use national_rank for D1-NAIA, cpi_rank for JUCO
        aVal = a.division === 'JUCO' ? a.cpi_rank : a.national_rank
        bVal = b.division === 'JUCO' ? b.cpi_rank : b.national_rank
      } else {
        aVal = a[sortKey]
        bVal = b[sortKey]
      }
            ...
            {filtered.map((team, i) => (
              ...
              >
                ...
                <td className="px-3 py-2 text-center text-xs font-semibold text-gray-700 dark:text-gray-300 whitespace-nowrap">
                  {team.division === 'JUCO'
                    ? (team.cpi_rank != null ? <span title="CPI rank (NWAC internal)">CPI #{team.cpi_rank}</span> : '-')
