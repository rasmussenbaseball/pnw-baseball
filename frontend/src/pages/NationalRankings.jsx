...
function percentileColor(pct) {
  ...
  if (pct >= 50) return 'text-gray-700 dark:text-gray-300'
...
function PercentileBar({ value }) {
  if (!value && value !== 0) return <span className="text-gray-400 dark:text-gray-500 text-xs">-</span>
...
function SourceBadge({ source, rank, total }) {
  ...
  return (
    <span className={`inline-flex items-center px-2 py-0.5 rounded text-xs font-medium border ${colors[source] || 'bg-gray-100 dark:bg-gray-700 text-gray-700 dark:text-gray-300 border-gray-200 dark:border-gray-700'}`}>
      {labels[source] || source} #{rank}
      {total ? <span className="text-gray-400 dark:text-gray-500 ml-0.5">/{total}</span> : null}
    </span>
  )
}
...
function SosRankBadge({ rank, label }) {
  ...
  return (
    <span className="text-xs text-gray-500 dark:text-gray-400" title={`${label} SOS Rank`}>
    ...
    <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700 overflow-hidden">
      ...
      >
        <div className="flex items-center gap-3">
          ...
          <h3 className="text-lg font-bold text-gray-900 dark:text-gray-100">{division.division_name}</h3>
          <span className="text-sm text-gray-400 dark:text-gray-500">({division.teams.length} teams)</span>
        </div>
        <svg className={`w-5 h-5 text-gray-400 dark:text-gray-500 transition-transform ${expanded ? 'rotate-180' : ''}`}
      ...
      {expanded && hasData && (
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            <thead>
              <tr className="text-xs text-gray-500 dark:text-gray-400 uppercase tracking-wider border-t border-b border-gray-100 dark:border-gray-700 bg-gray-50 dark:bg-gray-900/40/50">
                    ...
                    <div className="flex items-center gap-2.5">
                      ...
                      <div>
                        <Link to={`/team/${team.team_id}`}
                              className="font-semibold text-gray-900 dark:text-gray-100 hover:text-nw-teal transition-colors">
                          {team.short_name}
                        </Link>
                        <p className="text-xs text-gray-400 dark:text-gray-500">{team.conference_name}</p>
                      </div>
                    </div>
                  </td>
            ...
            <tbody>
              {division.teams.map((team, idx) => (
                <tr key={team.team_id} className="border-b border-gray-50 hover:bg-nw-cream/30">
                  <td className="px-5 py-2.5 text-gray-400 dark:text-gray-500 font-medium">{idx + 1}</td>
                  <td className="px-3 py-2.5">
                    <div className="flex items-center gap-2">
                      {team.logo_url && <img src={team.logo_url} alt="" className="w-6 h-6 object-contain" />}
                      <Link to={`/team/${team.team_id}`}
                            className="font-medium text-gray-900 dark:text-gray-100 hover:text-nw-teal">
                        {team.short_name}
                      </Link>
                    </div>
                  </td>
  ...
  return (
    <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700 overflow-hidden">
      <div className="px-5 py-4 border-b border-gray-100 dark:border-gray-700">
        <h3 className="text-lg font-bold text-gray-900 dark:text-gray-100">Cross-Division Comparison</h3>
      ...
      <div className="overflow-x-auto">
        <table className="w-full text-sm">
          <thead>
            <tr className="text-xs text-gray-500 dark:text-gray-400 uppercase tracking-wider border-b border-gray-100 dark:border-gray-700 bg-gray-50 dark:bg-gray-900/40/50">
              <th className="text-left px-5 py-2.5 w-8">#</th>
              <th className="text-left px-3 py-2.5">Team</th>
          ...
          <tbody>
            {teams.map((team, idx) => {
              const badgeClass = BADGE_COLORS[team.division_level] || 'bg-gray-50 dark:bg-gray-900/400 text-white'
              return (
                <tr key={team.team_id}
                    className={`border-b border-gray-50 hover:bg-nw-cream/30 ${percentileBg(team.national_percentile || 0)}`}>
                  <td className="px-5 py-2.5 font-bold text-gray-400 dark:text-gray-500">{idx + 1}</td>
                  <td className="px-3 py-2.5">
                    <div className="flex items-center gap-2">
                      {team.logo_url && <img src={team.logo_url} alt="" className="w-6 h-6 object-contain" />}
                      <Link to={`/team/${team.team_id}`}
                            className="font-semibold text-gray-900 dark:text-gray-100 hover:text-nw-teal">
                        {team.short_name}
                      </Link>
                    </div>
                  </td>
  ...
  return (
    <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700 overflow-hidden">
      <button
        ...
        className="w-full flex items-center justify-between px-5 py-3 hover:bg-gray-50 dark:bg-gray-900/40 transition-colors"
      >
        <span className="text-sm font-semibold text-gray-700 dark:text-gray-300">How National Rankings Work</span>
      ...
      {open && (
        <div className="px-5 pb-4 text-sm text-gray-600 dark:text-gray-400 space-y-3 border-t border-gray-100 dark:border-gray-700 pt-3">
          <p>
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4">
        <div>
          <h1 className="text-3xl font-extrabold text-gray-900 dark:text-gray-100 tracking-tight">National Rankings</h1>
          ...
          <div className="flex bg-gray-100 dark:bg-gray-700 rounded-lg p-0.5">
            <button
              ...
              className={`px-4 py-2 text-sm font-medium rounded-md transition-colors ${
                view === 'by-division' ? 'bg-white dark:bg-gray-800 shadow text-gray-900 dark:text-gray-100' : 'text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:text-gray-300'
              }`}
            >
              By Division
            </button>
            <button
              ...
              className={`px-4 py-2 text-sm font-medium rounded-md transition-colors ${
                view === 'cross-division' ? 'bg-white dark:bg-gray-800 shadow text-gray-900 dark:text-gray-100' : 'text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:text-gray-300'
              }`}
            >
              Cross-Division
            </button>
      ...
      {!hasCompositeData && divisions.length === 0 && (
        <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700 p-8 text-center">
          <p className="text-gray-500 dark:text-gray-400 text-lg mb-2">No national ranking data yet</p>
      ...
      {/* Data sources footer */}
      <div className="text-xs text-gray-400 dark:text-gray-500 text-center pt-4 space-y-1">
