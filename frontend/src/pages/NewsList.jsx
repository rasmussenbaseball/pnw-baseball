...
export default function NewsList() {
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 py-8">
      ...
      {loading && (
        <div className="text-gray-500 dark:text-gray-400 animate-pulse">Loading articles…</div>
      )}
      {error && (
        <div className="text-rose-600 dark:text-rose-400">Could not load articles: {error}</div>
      )}
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
        {articles.map(a => (
          <Link
            key={a.id}
            to={`/news/${a.slug}`}
            className="group rounded-xl overflow-hidden bg-white dark:bg-gray-800
